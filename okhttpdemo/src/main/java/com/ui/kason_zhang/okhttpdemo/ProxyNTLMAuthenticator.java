...
public class ProxyNTLMAuthenticator implements Authenticator {
    ...
    public Request authenticate(Route route, Response response) throws IOException {
        ...
        final List<String> authHeader = response.headers().values(PROXY_AUTHENTICATE);
        ...
        if (authHeader.contains(PROXY_AUTHENTICATE_TYPE)) {
            ...
            Request request = response.request().newBuilder()
                    .header(PROXY_AUTHORIZATION, buildProxyAuthHttpHeader(type1Msg))
                    .build();
        ...
        Request request = response.request().newBuilder()
                .header(PROXY_AUTHORIZATION, buildProxyAuthHttpHeader(type3Msg))
                .build();
